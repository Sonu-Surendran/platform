...
import { fetchMicPom } from '@/services/api';
...
const DetailRow = ({ label, value, icon }: { label: string; value?: string | number; icon?: React.ReactNode }) => (
  <div className="flex flex-col">
    ...
    <span className="font-medium text-slate-800 dark:text-charcoal-50 text-sm">{value || '-'}</span>
  </div>
);
...
export const MicPom: React.FC = () => {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        const result = await fetchMicPom();
        setData(result);
      } catch (e) {
        console.error("Failed to fetch MIC POM data", e);
      }
    };
    loadData();
  }, []);
