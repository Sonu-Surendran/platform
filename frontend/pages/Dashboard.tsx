...
import { fetchDeals, fetchDashboardStats } from '@/services/api';
...
export const Dashboard: React.FC<DashboardProps> = ({ onNavigateToDeal, onFilterDeals }) => {
  ...
  // --- Pipeline Data Calculation (From DB or Fallback) ---
  const pipelineData = useMemo(() => {
    if (dashboardData.pipeline && dashboardData.pipeline.length > 0) {
      return dashboardData.pipeline;
    }
    // Fallback if DB is empty for demo
  ...
  // --- Revenue Data (From DB or Fallback) ---
  const revenueData = useMemo(() => {
    if (dashboardData.revenue_trend && dashboardData.revenue_trend.length > 0) {
  ...
  const regionData = useMemo(() => {
    if (dashboardData.regional_sales && dashboardData.regional_sales.length > 0) return dashboardData.regional_sales;
    return [];
  }, [dashboardData]);
  ...
  const pendingReviews = useMemo(() => deals.filter(d => d.micStatusAccess === 'Review'), [deals]);
  ...
  return (
    <div className="h-full relative overflow-hidden flex flex-col">
      <div className="flex-1 overflow-y-auto custom-scrollbar p-8 space-y-8">
        ...
        {/* Using Data from Dashboard Stats Summary */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <StatCard
            title="Total Active Pipeline"
            value={`$${(dashboardData.summary?.total_tcv || 0).toLocaleString()}`}
            trend="12.5%"
            trendUp={true}
          ...
          />
          <StatCard
            title="Forecasted Revenue (ACV)"
            value={`$${(dashboardData.summary?.total_acv || 0).toLocaleString()}`}
            trend="8.2%"
            trendUp={true}
          ...
          />
          <StatCard
            title="Active Deals Count"
            value={`${(dashboardData.summary?.total_deals || 0)}`}
            trend="--"
            trendUp={true}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 auto-rows-[minmax(180px,auto)] pb-8">
          ...
          <GlassCard className="col-span-1 md:col-span-2 lg:col-span-2 row-span-2 h-[400px]" title="Revenue Trend & Forecast">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={revenueData}>
                ...
                />
                <Area type="monotone" dataKey="value" stroke="#00ADB5" strokeWidth={3} fillOpacity={1} fill="url(#colorValue)" name="ACV (Mic)" />
                <Area type="monotone" dataKey="value2" stroke="#C9E9D2" strokeWidth={3} fillOpacity={1} fill="url(#colorValue2)" name="TCV (MCN)" />
          ...
          <GlassCard className="col-span-1 md:col-span-2 lg:col-span-2 row-span-2 h-[400px]" title="Pipeline by Stage" action={<button className="text-slate-400 hover:text-slate-600 dark:hover:text-charcoal-50"><MoreHorizontal size={20} /></button>}>
            <ResponsiveContainer width="100%" height="100%">
          ...
          {/* SMALL CHART 1: Product Mix (Using Real Data) */}
          <GlassCard className="col-span-1 h-[280px]" title="Request Types">
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                <Pie
                  data={dashboardData.product_mix || []}
                  innerRadius={60}
                  outerRadius={80}
                  paddingAngle={5}
                  dataKey="value"
                  stroke="none"
                >
                  {(dashboardData.product_mix || []).map((entry: any, index: number) => (
                    <Cell key={`cell-${index}`} fill={COLORS[index % COLORS.length]} />
                  ))}
                </Pie>
          ...
          {/* SMALL CHART 2: Regional Sales (Using Real Data) */}
          <GlassCard className="col-span-1 h-[280px]" title="Regional ACV">
            <ResponsiveContainer width="100%" height="100%">
