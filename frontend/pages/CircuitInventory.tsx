...
// Removed XLSX import as it was only used for import
...
export const CircuitInventory: React.FC = () => {
  ...
  // Removed localStorage loading and File Upload handlers
  ...
  const filteredData = useMemo(() => {
    ...
    return inventoryData.filter(item => {
      ...
      const matchesSearch = searchQuery === '' || Object.values(item as any).some(val =>
        val !== null && val !== undefined && String(val).toLowerCase().includes(lowerQuery)
      );

      const matchesType = selectedType === 'All' || item.circuit_type === selectedType;
      const matchesCountry = selectedCountry === 'All' || item.country === selectedCountry;
      const matchesCarrier = selectedCarrier === 'All' || item.carrier_partner === selectedCarrier;
  ...
  const uniqueTypes = ['All', ...new Set(inventoryData.map(item => item.circuit_type).filter(Boolean))];
  const uniqueCountries = ['All', ...new Set(inventoryData.map(item => item.country).filter(Boolean))];
  const uniqueCarriers = ['All', ...new Set(inventoryData.map(item => item.carrier_partner).filter(Boolean))];
          ...
          <table className="w-full text-left whitespace-nowrap">
            <thead className={`bg-slate-50/80 dark:bg-charcoal-900/80 sticky top-0 z-10 backdrop-blur-md ${isFocusMode ? 'shadow-sm' : ''}`}>
              <tr className="border-b border-slate-200 dark:border-charcoal-700">
                {[
                  'Client', 'Address', 'City', 'Country', 'Carrier Partner',
                  'Circuit Type', 'DL Mbps', 'UL Mbps', 'MRC (USD)', 'NRC (USD)', 'Term', 'Availability SLA', 'Actions'
                ].map((header) => (
              ...
              {currentData.length > 0 ? (
                currentData.map((circuit) => (
                  ...
                  >
                    ...
                    <td className="px-3 py-2 md:px-6 md:py-4 text-xs md:text-sm font-medium text-slate-800 dark:text-charcoal-50">
                      ${circuit.mrc_usd?.toLocaleString()}
                    </td>
                    <td className="px-3 py-2 md:px-6 md:py-4 text-xs md:text-sm text-slate-600 dark:text-gray-400">
                      ${circuit.nrc_usd?.toLocaleString()}
                    </td>
                    <td className="px-3 py-2 md:px-6 md:py-4 text-xs md:text-sm text-slate-600 dark:text-gray-400">
                      {circuit.term}
                    </td>
              ...
              {Array.from({ length: Math.min(5, totalPages) }, (_, i) => {
                ...
                return (
                  <button
                    ...
                    className={`h-7 w-7 md:h-8 md:w-8 rounded-lg text-xs md:text-sm font-medium transition-colors ${currentPage === pageNum
            ...
            <div className="p-4 md:p-8 space-y-4 md:space-y-8 bg-slate-50/30 dark:bg-charcoal-900/30 flex-1 overflow-y-auto custom-scrollbar">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
                ...
                >
                  ...
                  <DetailField label="State" value={selectedCircuit.state} />
                  <DetailField label="Zip/Post Code" value={selectedCircuit.zip} />
                  <DetailField label="Country" value={selectedCircuit.country} />
                  <DetailField label="Region" value={selectedCircuit.region} />
                  <DetailField label="Client" value={selectedCircuit.client} fullWidth />
                </DetailSection>
                ...
                >
                  <DetailField label="Circuit Type" value={selectedCircuit.circuit_type} />
                  <DetailField label="Quote Type" value={selectedCircuit.quote_type} />
                  <DetailField label="DL Mbps (DIA and BIA)" value={selectedCircuit.dl_mbps} highlight />
                  <DetailField label="UL Mbps (BIA Only)" value={selectedCircuit.ul_mbps} highlight />
                  <DetailField label="IP Range" value={selectedCircuit.ip_range} fullWidth />
                  <DetailField label="Carrier Partner" value={selectedCircuit.carrier_partner} />
                  <DetailField label="LMP" value={selectedCircuit.lmp} />
                </DetailSection>

                {/* Panel 3: Costs & Term */}
                <DetailSection
                  title="Financials & Term"
                ...
                >
                  <DetailField label="Currency" value={selectedCircuit.currency} />
                  <DetailField label="Term" value={selectedCircuit.term ? `${selectedCircuit.term} Months` : 'N/A'} />
                  <DetailField label="OTC" value={selectedCircuit.otc?.toLocaleString()} />
                  <DetailField label="MRC" value={selectedCircuit.mrc?.toLocaleString()} />
                  <DetailField label="NRC (USD)" value={selectedCircuit.nrc_usd?.toLocaleString()} highlight />
                  <DetailField label="MRC (USD)" value={selectedCircuit.mrc_usd?.toLocaleString()} highlight />
                  <DetailField label="Used in CPQ" value={selectedCircuit.used_in_cpq} fullWidth />
                </DetailSection>
              </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 md:gap-6">
                ...
                >
                  <div className="col-span-2 grid grid-cols-2 gap-4">
                    <DetailField label="Availability SLA" value={selectedCircuit.availability_sla} highlight />
                    <DetailField label="MTTR SLA" value={selectedCircuit.mttr_sla} />
                  </div>
                </DetailSection>
                ...
                >
                  ...
                  <DetailField label="CPQ Reference" value={selectedCircuit.used_in_cpq} />
                  <div className="col-span-2">
                    <p className="text-xs md:text-sm text-slate-600 dark:text-gray-300 bg-white dark:bg-charcoal-900 p-2 md:p-3 rounded-xl border border-slate-100 dark:border-charcoal-700 min-h-[60px]">
                      {selectedCircuit.notes || 'No notes provided.'}
                    </p>
                  </div>
                </DetailSection>
              </div>
