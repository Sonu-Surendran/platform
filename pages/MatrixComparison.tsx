    ...
    >
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 flex-none">
        ...
        <div className="flex gap-3">
          {/* History Controls */}
          <div className="flex items-center gap-1 bg-white dark:bg-charcoal-800 p-1 rounded-xl shadow-sm border border-slate-200 dark:border-charcoal-700 mr-2">
            <button 
              onClick={undo}
              disabled={!canUndo}
            ...
            <div className="w-px h-6 bg-slate-200 dark:bg-charcoal-700"></div>
            <button 
              onClick={redo}
              disabled={!canRedo}
              className="p-2 text-slate-600 dark:text-gray-300 hover:bg-slate-100 dark:hover:bg-charcoal-700 rounded-lg disabled:opacity-30 disabled:cursor-not-allowed transition-colors"
          ...
          <button 
            ...
            className="flex items-center gap-2 px-5 py-2.5 bg-pastel-blue dark:bg-charcoal-brand text-white font-medium rounded-xl shadow-lg hover:bg-opacity-90 transition-all"
          >
            <Upload size={18} /> Import Cost
          </button>
          
          <button 
            onClick={toggleFreeze}
            className={`flex items-center gap-2 px-5 py-2.5 font-medium rounded-xl transition-all shadow-lg text-white
              ${(freezeState.cols > 0) ? 'bg-sky-500 hover:bg-sky-600 shadow-sky-500/30' : 'bg-slate-500 hover:bg-slate-600 shadow-slate-500/30'}`}
            title={freezeState.cols > 0 ? "Unfreeze Columns" : "Freeze Selected Column(s)"}
          >
            <Snowflake size={18} /> {(freezeState.cols > 0) ? 'Unfreeze' : 'Freeze Cols'}
          </button>

          <button 
            onClick={handleScan}
            disabled={isScanning}
            className={`flex items-center gap-2 px-5 py-2.5 font-medium rounded-xl text-white shadow-lg transition-all
              ${isScanning ? 'bg-slate-400 cursor-wait' : 'bg-indigo-600 hover:bg-indigo-700 dark:bg-indigo-500 dark:hover:bg-indigo-600 shadow-indigo-500/30'}`}
          >
            {isScanning ? <><RefreshCw size={18} className="animate-spin" /> Scanning...</> : <><ScanLine size={18} /> Scan Matrix</>}
          </button>
        </div>
      </div>
      ...
      <div className="flex-1 flex flex-col bg-white/70 dark:bg-charcoal-800/70 backdrop-blur-xl border border-white/60 dark:border-charcoal-700/60 shadow-lg rounded-3xl overflow-hidden relative min-h-0">
        ...
        <div className="px-6 py-4 border-b border-slate-200 dark:border-charcoal-700 bg-slate-50/50 dark:bg-charcoal-900/30 flex items-center justify-between flex-none">
          ...
          <button onClick={handleExport} className="flex items-center gap-2 px-3 py-1.5 text-sm font-medium text-slate-600 dark:text-gray-300 hover:bg-white dark:hover:bg-charcoal-700 rounded-lg transition-colors border border-transparent hover:border-slate-200">
