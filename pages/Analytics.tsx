...
export const Analytics: React.FC = () => {
  ...
  // Load Data
  useEffect(() => {
    const loadedCircuits = localStorage.getItem('circuit_inventory_data');
    const loadedDeals = localStorage.getItem('deals_data');

    if (loadedCircuits) {
      try { setCircuits(JSON.parse(loadedCircuits)); } catch (e) { console.error(e); }
    }
    if (loadedDeals) {
      try { setDeals(JSON.parse(loadedDeals)); } catch (e) { console.error(e); }
    }
  }, []);
  ...
  const handleUploadSubmit = (formData: any, invoiceName: string, contractName: string) => {
    ...
    // Simulate API call and Local Persistence for MIC POM
    setTimeout(() => {
      const newRecord: MicPomRecord = {
        id: `POM-${Date.now()}`,
        ...formData,
        mrc: parseFloat(formData.mrc) || 0,
        nrc: parseFloat(formData.nrc) || 0,
        invoiceFile: invoiceName || `INV-${Date.now().toString().substr(-4)}.pdf`,
        contractFile: contractName || `CTR-${Date.now().toString().substr(-4)}.pdf`,
        timestamp: new Date().toISOString()
      };

      // Get existing data
      const existingData = localStorage.getItem('mic_pom_data');
      const pomData = existingData ? JSON.parse(existingData) : [];
      const updatedData = [newRecord, ...pomData];

      // Save
      localStorage.setItem('mic_pom_data', JSON.stringify(updatedData));

      setIsSubmitting(false);
      setIsUploadModalOpen(false);
      alert("Billing data uploaded successfully to MIC POM!");
    }, 1500);
  };
